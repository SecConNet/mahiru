...
class AssetConnections:
    """Whether and how to serve incoming connections to assets.
    ...
    def __init__(
            ...
            net_admin_helper_path: Optional[Path] = None
            ) -> None:
        ...
        Args:
            ...
            ports: Port range to use for serving incoming connections,
                as a length-2 list of the form [min, max].
            net_admin_helper_path: Local path to the net-admin-helper
                binary.
        """
        if enabled:
            if not external_ip:
            ...
            if not net_admin_helper_path:
                raise RuntimeError('No net-admin-helper path specified')

        self.enabled = enabled
        self.external_ip = external_ip
        self.ports = ports
        self.net_admin_helper_path = net_admin_helper_path
...
class Settings:
    ...
    Attributes:
        ...
        asset_connections: Configuration for external asset connections.
    ...
    def __init__(
            ...
            asset_connections: AssetConnections,
        ...
        Args:
            ...
            asset_connections: Configuration for external asset
                    connections.
            registry_endpoint: Registry endpoint location.
            loglevel: Logging level to use, one of 'critical', 'error',
                    'warning', 'info', or 'debug'.
        ...
        self.owner = owner
        self.asset_connections = asset_connections
    ...
    def _yatiml_savorize(cls, node: yatiml.Node) -> None:
        if node.is_mapping():
            if not node.has_attribute('asset_connections'):
                ac_node = yaml.MappingNode('tag:yaml.org,2002:map', [])
                node.set_attribute('asset_connections', ac_node)


_load_settings = yatiml.load_function(Settings, AssetConnections, Identifier)
